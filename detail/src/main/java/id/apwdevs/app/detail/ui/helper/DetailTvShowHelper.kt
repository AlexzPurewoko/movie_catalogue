...
import id.apwdevs.app.detail.databinding.FragmentDetailTvShowBinding
...
class DetailTvShowHelper(
    ...
    override fun initView() {
        contentBinding = ContentDetailTvshowBinding.inflate(LayoutInflater.from(rootBinding.root.context), rootBinding.nestedScroll, false)
    ...
    private fun composeFrame(data: TvShowDetail) {
        val nextEpisodeFg = data.nextEpisodeToAir?.let { EpisodeItem.newInstance(it) } ?: NoDataItem()
        val lastEpisodeFg = data.lastEpisodeToAir.let { EpisodeItem.newInstance(it) }
        val backdropImg = data.backdropPath?.let { CardBackdropItem.newInstance(it.getImageURL()!!, data.title) } ?: NoDataItem()
