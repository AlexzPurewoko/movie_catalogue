...
class MovieShowViewModel(
    ...
    private fun displayPopularMovie(): LiveData<PagingData<MovieShowItem>> {
        return popularUseCase.discoverPopularMovies()
            .map { pagingData ->
                pagingData.map {
                    MovieShowItem(it.movieId, it.title, it.overview, it.backdropPath, it.genres, it.voteCount, it.voteAverage)
                }
    ...
    private fun displayPopularTvShow(): LiveData<PagingData<MovieShowItem>> {
        return popularUseCase.discoverPopularTvShow()
            .map { pagingData ->
                pagingData.map {
                    MovieShowItem(it.tvId, it.name, it.overview, it.backdropPath, it.genres, it.voteCount, it.voteAverage)
                }
            }.asLiveData(viewModelScope.coroutineContext)
    }
}